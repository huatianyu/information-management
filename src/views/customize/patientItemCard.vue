    ...
    <div class="flex-container">
      ...
      <div class="flex-content">
        ...
        <div class="flex-content-bottom">
          性别：{{ item.sex == 0 ? '男' : '女' }} 出生日期：{{
            item.birthDay
          }}
          治疗方式：{{ TreatmentMethodForRead(item.treatmentMethod) }}
        </div>
        <div class="flex-content-bottom">
          登记时间：{{ item.creationTime | date('%Y-%m-%d %T') }}放疗号：{{
            item.radiotherapyNum
          }}
          治疗技师：{{ ChiefDoctorsForRead(item.chiefDoctorId) }}
        </div>
      </div>
