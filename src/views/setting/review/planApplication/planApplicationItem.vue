  ...
  <div style="margin-right:15px">
    <div class="flex-container">
      ...
      <div class="flex-content">
        ...
        > RTID:{{item.radiotherapyNum}} Date:{{item.homeDateTime | date('%Y-%m-%d %T')}}  状态:{{item.documentStatus}}  治疗技师：{{ChiefDoctorsForRead(item.chiefDoctorId)}}</div>
      </div>
      <div class="flex-content-btn">
        <Button @click="showpatientResource(item.id,item.radiotherapyNum)" type="primary">处理</Button>
      </div>
    </div>
  </div>
</template>
...
export default class PlanApplicationItem extends AbpBase {
