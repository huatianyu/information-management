<template>
  <div>
    <div class="flex-container" @click="showPatientDetail(item.locationId,item.radiotherapyNum)">
      <div class="flex-img">
...
export default class ProjectLocationDetailed extends AbpBase {
  ...
  showPatientDetail(locationId, radiotherapyNum) {
    this.$store.commit("targetResource/setRouterName", this.cardName);
    this.$store.dispatch(
      "targetResource/getExaminePatientListQueryAsync",
      { id: locationId, radiotherapyNum: radiotherapyNum }
    );
