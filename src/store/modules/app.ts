...
class AppModule implements Module<AppState, any>{
    ...
    mutations = {
        ...
        setDrawer(state: AppState, drawer: boolean) {
            state.drawer = drawer
        },
        // setImage(state: AppState) {
        //     state.image =  set('image'),
        // },

        // setColor(state: AppState) {
        //     state.color = 'color'
        // },

        // toggleDrawer(state: AppState) {
        //     state.toggleDrawer = toggle('drawer')
        // },

        ChangeTheme(state: AppState, themeImage: number) {
            state.themeImage = themeImage
        },
    ...
    actions = {
        ...
        async getDictionariesModelInfoAsync(content: ActionContext<AppState, any>, payload: any) {

            //防止刷新页面数据消失

            content.state.rtisDict = JSON.parse(localStorage.getItem("content.state.rtisDict"));

            let reponse = await ajax.post('/api/services/app/RtisDict/QueryDictInfoAsync');
            if (reponse.data["success"]) {
                localStorage.setItem("content.state.rtisDict", JSON.stringify(reponse.data.result));
                content.state.rtisDict = reponse.data.result
            }
