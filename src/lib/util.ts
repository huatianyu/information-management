...
class Util {
    ...
    openNewPage(vm: Vue, name: string | undefined, argu?: any, query?: any) {
        ...
        if (!tagHasOpened) {
            ...
            if (!tag) {
                ...
                vm.$store.state.app.tagsList.forEach((item: any) => {
                    if (item.children) {
    ...
    //刷新页面也能获取数据
    getUpdatePageObj(obj: any, pageName: string, id: string) {
        if (obj && JSON.stringify(obj) != "{}" && id) {
            localStorage.setItem(pageName, JSON.stringify(obj));
            return obj;
        } else {
